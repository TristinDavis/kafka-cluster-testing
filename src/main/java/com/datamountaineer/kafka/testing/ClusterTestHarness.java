package com.datamountaineer.kafka.testing;
...
public abstract class ClusterTestHarness {
  ...
  public static int[] choosePorts(int count) {
    try {
      ...
      for (int i = 0; i < count; i++) {
        sockets[i] = new ServerSocket(0, 0, InetAddress.getLocalHost());
  ...
  public void setUp() throws Exception {
    int[] ports = new int[numBrokers + 1];
    ...
    for (int i = 0; i < numBrokers; i++) {
      ...
      servers.add(server);
      ports[i] = server.boundPort(SecurityProtocol.PLAINTEXT);
    }
    ...
    if (setupRestApp) {
      ...
      restApp = new RestApp(schemaRegPort, zkConnect, KAFKASTORE_TOPIC, compatibilityType);
      restApp.start();
      ports[numBrokers] = schemaRegPort;
    }
    this.ports =ports;
  }
